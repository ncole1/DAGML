def lint(dagIndents, indentSize, dagItem, dagFlows):
    ...
    for i in range(0, len(dagItem)):
        if type(dagIndents[i]) != int:
            raise Exception('Error: dagIndents element number '
                            + str(i) + ' is not of type \'int\'.')
        if len(dagItem[i]) == 0:
            raise Exception('Dag items may not be empty, and blank lines are forbidden. Error on line '
                            + str(i))
        if dagItem[i][0] == ' ':
            raise Exception('Dag items may not begin with a space. Error on line '
                            + str(i))
        if dagItem[i][-1] == ' ':
            raise Exception('Dag items may not end with a space. Error on line '
                            + str(i))
        if str(dagFlows[i])[0] == ' ':
            raise Exception('Dag flows may not begin with a space. Error on line '
                            + str(i))
        if str(dagFlows[i])[-1] == ' ':
            raise Exception('Dag flows may not end with a space. Error on line '
                            + str(i))
        for j in range(0, len(dagFlows[i])-1):
            if dagFlows[i][j:j+2] == '::':
                raise Exception('Dag flows may not have double or multiple colons. Error on line '
                                + str(i))
        for j in range(0, len(dagItem[i])):
            ...
            if (dagItem[i][j] == '(' or dagItem[i][j] == ')' or dagItem[i][j] == '[' or dagItem[i][j] == ']') and (not pokay):
                raise Exception('Dag items may not contain parentheses or brackets. Error on line '
                                + str(i))
            elif (pokay and (pchar != dagItem[i][j])):
                raise Exception('Missing or improper merge parentheses on line '
                                + str(i))
    for i in range(0, len(dagIndents)):
        ...
        for j in range(0, len(itemPlusFlow)):
            if ((itemPlusFlow[j] != '\'') and ((itemPlusFlow[j] != ' ') or j < len(dagItem[i]))):
    ...
    for i in range(0, len(fileForm)):
        ...
        if (not mergeLine):
            ...
            if (itemAndFlow[itemEndChar:len(itemAndFlow)]).count('|') == 0:
                ...
                newFlows.append(literal,
                                tuple(newFlowItems[1:len(newFlowItems)]))
        else:
            ...
            if lineLength > 7 and i > 0:
                ...
                for j in range(0, commas):
                    ...
                    if nextComma > (charIndex + 2):
                        ...
                        raise Exception('Invalid spacing between commas in merge on line '
                                        + str(i))
                    operFound = False
                    for k in range(0, i):
                        if newItem[k] == ('<' + operString):
                            operFound = True
                    if (not operFound):
                        raise Exception('Error: operator \'' + operString
                                        + '\' not found prior to being involved in a merge on line '
                                        + str(i))
                print('End')
            else:
                raise Exception('Error: Invalid or empty merge on line '
                                + str(i))
    ...
    for i in range(0, len(newFlows)):
        if newIndents[i] != dagIndents[i]:
            raise Exception('Error: Incorrect new number of indents on line '
                            + str(i))
        if newItem[i] != dagItem[i]:
            raise Exception('Error: Incorrect new dag item on line '
                            + str(i))
